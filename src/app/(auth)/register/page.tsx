...
import { AgeGroupUser }        from '../../../types';
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      await register({ name, email, password, ageGroup, consentGiven: consent });
      router.push('/chat');
    } catch {
  ...
  const needsConsent = ageGroup === '10-13';

  const inputStyle: React.CSSProperties = {
    width: '100%', padding: '10px 13px', borderRadius: 9,
    background: 'var(--surface-input)', border: '1px solid var(--border-input)',
    color: 'var(--accent-text)', fontSize: 16, fontFamily: 'DM Sans, sans-serif',
    outline: 'none', boxSizing: 'border-box',
  };

  const optionBtnStyle = (active: boolean): React.CSSProperties => ({
    flex: 1, minWidth: 0, padding: '9px 8px', borderRadius: 8,
    background:  active ? 'var(--accent-dim)' : 'var(--surface-input)',
    border:      `1px solid ${active ? 'rgba(74,222,128,0.3)' : 'var(--border-faint)'}`,
  ...
  return (
    <div className="flex justify-center px-4 py-8 sm:py-12" style={{ background: 'var(--bg)', minHeight: '100dvh' }}>
      <div className="w-full max-w-[420px] rounded-2xl p-5 sm:p-8 h-fit"
        style={{ background: 'var(--surface)', border: '1px solid var(--border-input)' }}>

        {/* Brand */}
        <div className="flex items-center gap-3 mb-6 sm:mb-7">
          <div className="w-[42px] h-[42px] rounded-full flex items-center justify-center font-black text-[19px] shrink-0"
            style={{ background: 'var(--accent-gradient)', color: '#09160d' }}>A</div>
          <div>
        ...
        <div className="font-bold text-[18px] sm:text-[20px] mb-1" style={{ color: 'var(--txt-1)' }}>Create your account</div>
        <div className="text-[13px] mb-5 sm:mb-6" style={{ color: 'var(--txt-3)' }}>
          Save your chats, track your progress, and stay safe.
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {/* Age group */}
          <div className="mb-4">
            <label className="block text-[11.5px] mb-2" style={{ color: 'var(--txt-3)' }}>Age group</label>
          ...
          {needsConsent && (
            <div className="mb-4 flex items-start gap-3">
              ...
              <label htmlFor="consent" className="text-[12px] leading-relaxed" style={{ color: 'var(--txt-3)' }}>
          ...
          <button type="submit" disabled={loading || (needsConsent && !consent)}
            ...
            style={{
              background: 'var(--accent-gradient)', color: '#09160d',
        ...
        <div className="text-center mt-5 text-[13px]" style={{ color: 'var(--txt-3)' }}>
          Already have an account?{' '}
          <Link href="/login" style={{ color: 'var(--accent)' }}>Sign in</Link>
        </div>

        <div className="text-center mt-3 text-[11.5px]" style={{ color: 'var(--txt-5)' }}>
          Facilitator?{' '}
          <span style={{ color: 'var(--txt-4)' }}>Register using your invite link.</span>
        </div>
      </div>
    </div>
  );
